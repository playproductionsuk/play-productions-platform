...
function renderMusicLibrary(){
  ...
  target.querySelectorAll("#musicCatalogueTable tbody tr").forEach(row=>[8,9,11].forEach(index=>{const cell=row.children[index];if(!cell)return;const enabled=cell.textContent.trim()==="✓";cell.innerHTML=`<span class="library-state ${enabled?"on":"off"}" aria-label="${enabled?"Enabled":"Disabled"}">${enabled?"✓":"×"}</span>`}));
