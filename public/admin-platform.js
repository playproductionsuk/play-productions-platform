...
function markRequiredFields(fields,message){document.querySelectorAll(".field-required").forEach(node=>node.classList.remove("field-required"));let first=null;fields.forEach(field=>{const targetId=missingFieldTargets[field]||field,control=document.querySelector(`#${targetId}`),wrap=control?.closest(".field,.file-field,.check-field,label")||control;if(wrap){wrap.classList.add("field-required");if(!first)first=control||wrap}});const section=first?.closest?.(".track-editor-section");if(section)section.open=true;if(first){first.scrollIntoView({behavior:"smooth",block:"center"});first.focus?.()}if(progress&&message)progress.textContent=message}
function setSaveStage(message){if(progress)progress.textContent=message}
function setAssetProgress(inputId,label,percent,message){const card=document.querySelector(`#${inputId}`)?.closest(".file-field"),meter=card?.querySelector("progress"),text=card?.querySelector(".asset-progress-text");if(!card||!meter||!text)return;card.classList.add("is-uploading");meter.hidden=false;if(Number.isFinite(percent)){meter.value=Math.max(0,Math.min(100,percent));text.textContent=`${message||`Uploading ${label}`} ${Math.round(meter.value)}%`}else{text.textContent=message||`Preparing ${label} upload…`}}
function finishAssetProgress(inputId,label,message){const card=document.querySelector(`#${inputId}`)?.closest(".file-field"),meter=card?.querySelector("progress"),text=card?.querySelector(".asset-progress-text");if(!card||!meter||!text)return;card.classList.remove("is-uploading");meter.hidden=false;meter.value=100;text.textContent=message||`${label} upload complete`}
function clearAssetProgress(){document.querySelectorAll(".file-field").forEach(card=>{card.classList.remove("is-uploading");const meter=card.querySelector("progress"),text=card.querySelector(".asset-progress-text");if(meter){meter.hidden=true;meter.value=0}if(text)text.textContent=""})}
function setTrackFormBusy(busy){document.querySelector("#trackForm")?.classList.toggle("is-saving",busy);document.querySelectorAll(".asset-file-input").forEach(input=>input.disabled=busy);document.querySelectorAll(".asset-upload-action,.asset-remove-action").forEach(control=>{control.toggleAttribute("aria-disabled",busy);if(control.tagName==="BUTTON"){if(busy){control.dataset.wasDisabled=control.disabled?"true":"false";control.disabled=true}else control.disabled=control.dataset.wasDisabled==="true"}})}
let librarySort={key:"",direction:1};
...
function sortMusicLibrary(button){
  const key=button.dataset.librarySort,type=button.dataset.sortType||"text";
  librarySort={key,direction:librarySort.key===key?-librarySort.direction:1};
  const tbody=document.querySelector("#musicCatalogueTable tbody");
  if(!tbody)return;
  const rows=[...tbody.querySelectorAll("tr")];
  rows.sort((a,b)=>{
    const left=a.dataset[key]??"",right=b.dataset[key]??"";
    const leftEmpty=left==="",rightEmpty=right==="";
    if(leftEmpty!==rightEmpty)return leftEmpty?1:-1;
    const result=type==="number"?Number(left)-Number(right):left.localeCompare(right,undefined,{numeric:true,sensitivity:"base"});
    return result*librarySort.direction;
  });
  rows.forEach(row=>tbody.appendChild(row));
  document.querySelectorAll("[data-library-sort]").forEach(header=>{
    const active=header===button;
    header.classList.toggle("active",active);
    header.setAttribute("aria-sort",active?(librarySort.direction===1?"ascending":"descending"):"none");
  });
}
function renderMusicLibrary(){
  ...
  const counts=state.tracks.reduce((result,track)=>{
    ...
    if(archived){result.archived++;return result}
    result.all++;
    if(track.showInStore)result.store++;
    if(track.showInDjPool)result.djPromo++;
    if(track.purchaseEnabled)result.purchase++;
    if(track.featured)result.featured++;
    if(track.showInLatest)result.latest++;
    return result;
  },{all:0,store:0,djPromo:0,purchase:0,featured:0,latest:0,archived:0});
  counts["missing-data"]=missingRows.length;
  const filterLabels={all:"All",store:"Store","dj-promo":"DJ Promo",purchase:"Purchase",featured:"Featured",latest:"Latest",archived:"Archived","missing-data":"Missing Data"};
  const countKeys={all:"all",store:"store","dj-promo":"djPromo",purchase:"purchase",featured:"featured",latest:"latest",archived:"archived","missing-data":"missing-data"};
  const libraryActions=document.querySelector(".music-library-top-actions");
  if(filters)filters.innerHTML=["all","store","dj-promo","purchase","featured","latest","archived","missing-data"].map(key=>`<button type="button" class="${libraryFilter===key?"active":""}" data-music-filter="${key}" title="${key==="missing-data"?"Missing field items":`${filterLabels[key]} tracks currently enabled`}">${filterLabels[key]} <span>${counts[countKeys[key]]}</span></button>`).join("");
  if(filters&&libraryActions)filters.appendChild(libraryActions);
  const catalogue=state.tracks.length?`<table id="musicCatalogueTable" class="library-table music-library-table"><thead><tr><th>Artwork</th><th><button data-library-sort="title">Track</button></th><th><button data-library-sort="genre">Genre / subgenre</button></th><th><button data-library-sort="bpm" data-sort-type="number">BPM</button></th><th><button data-library-sort="key">Key</button></th><th><button data-library-sort="mood">Mood</button></th><th><button data-library-sort="price" data-sort-type="number">Price</button></th><th><button data-library-sort="status">Status</button></th><th><button data-library-sort="store" data-sort-type="number">Store</button></th><th><button data-library-sort="djPromo" data-sort-type="number">DJ</button></th><th><button data-library-sort="purchase" data-sort-type="number">Purchase</button></th><th><button data-library-sort="featured" data-sort-type="number">Featured</button></th><th><button data-library-sort="latest" data-sort-type="number">Latest</button></th><th><button data-library-sort="health" data-sort-type="number">Health</button></th><th>Actions</th></tr></thead><tbody>${state.tracks.map(track=>{
    const health=trackHealth(track),readiness=trackReadiness(track),archived=track.status==="archived",docId=trackDocumentId(track);
    ...
    const keys=[...(!archived?["all"]:[]),...(!archived&&track.showInStore?["store"]:[]),...(!archived&&track.showInDjPool?["dj-promo"]:[]),...(!archived&&track.purchaseEnabled?["purchase"]:[]),...(!archived&&track.featured?["featured"]:[]),...(!archived&&track.showInLatest?["latest"]:[]),...(archived?["archived"]:[])];
    const archiveAction=archived?`<button type="button" class="track-restore-button" data-track-restore="${escapeHtml(docId)}">Restore</button>`:`<button type="button" class="track-archive-button" data-track-archive="${escapeHtml(docId)}">Archive</button>`;
    const healthState=health.missingRequired.length?"red":health.score<100?"amber":"green";
    const healthTitle=[...health.missingRequired,...health.missingRecommended].join(", ")||"Complete";
    return`<tr data-track-row="${escapeHtml(docId)}" data-library-filters="${keys.join(" ")}" data-title="${escapeHtml(track.title)}" data-genre="${escapeHtml(`${track.style||""} ${track.subgenre||""}`.trim())}" data-bpm="${track.bpm||""}" data-key="${escapeHtml(track.key||"")}" data-mood="${escapeHtml((track.moodTags||[]).join(", "))}" data-price="${Number(track.price||0)}" data-status="${escapeHtml(track.status||"draft")}" data-store="${track.showInStore?1:0}" data-dj-promo="${track.showInDjPool?1:0}" data-purchase="${track.purchaseEnabled?1:0}" data-featured="${track.featured?1:0}" data-latest="${track.showInLatest?1:0}" data-health="${health.score}"><td><img src="${escapeHtml(track.coverUrl||"icons/fallback.png")}" alt=""></td><td><strong>${escapeHtml(track.title)}</strong><small>${escapeHtml(track.artist||"Play Productions")}</small></td><td>${escapeHtml(track.style||"—")}<small>${escapeHtml(track.subgenre||"")}</small></td><td>${track.bpm||"—"}</td><td>${escapeHtml(track.key||"—")}</td><td>${escapeHtml((track.moodTags||[]).join(", ")||"—")}</td><td>£${Number(track.price||0).toFixed(2)}</td><td><span class="track-status-value">${escapeHtml(track.status||"draft")}</span></td><td>${track.showInStore?"✓":"×"}</td><td>${track.showInDjPool?"✓":"×"}</td><td>${track.purchaseEnabled?"✓":"×"}</td><td>${track.featured?"✓":"×"}</td><td>${track.showInLatest?"✓":"×"}</td><td><button type="button" class="health-percent ${healthState}" data-edit="${escapeHtml(docId)}" title="${escapeHtml(healthTitle)}">${health.score}%</button></td><td><div class="track-library-actions"><button type="button" class="button ghost library-edit-button" data-edit="${escapeHtml(docId)}">Update / edit</button>${archiveAction}<button type="button" class="track-delete-button" data-track-delete-permanent="${escapeHtml(docId)}">Delete</button></div></td></tr>`;
  }).join("")}</tbody></table>`:`<p class="empty">No tracks yet.</p>`;
  const missingTable=`<table id="musicMissingDataTable" class="library-table missing-data-table" hidden><thead><tr><th>Priority</th><th>Track</th><th>Area</th><th>Missing field</th><th>Current value</th><th>Action</th></tr></thead><tbody>${missingRows.map(item=>`<tr><td><span class="missing-priority ${item.priority.toLowerCase()}">${item.priority}</span></td><td><strong>${escapeHtml(item.track.title||"Untitled track")}</strong></td><td>${item.area}</td><td>${escapeHtml(missingFieldLabels[item.field]||item.field)}</td><td><span class="missing-current">Missing</span></td><td><button type="button" class="button ghost missing-data-edit" data-missing-track="${escapeHtml(trackDocumentId(item.track))}" data-missing-area="${item.areaKey}" data-missing-field="${escapeHtml(missingFieldTargets[item.field]||item.field)}">Edit</button></td></tr>`).join("")||'<tr><td colspan="6" class="empty">No missing track data.</td></tr>'}</tbody></table>`;
  target.innerHTML=catalogue+missingTable;
  target.querySelectorAll("#musicCatalogueTable tbody tr").forEach(row=>[8,9,10,11,12].forEach(index=>{const cell=row.children[index];if(!cell)return;const enabled=cell.textContent.trim()==="✓";cell.innerHTML=`<span class="library-state ${enabled?"on":"off"}" aria-label="${enabled?"Enabled":"Disabled"}">${enabled?"✓":"×"}</span>`}));
...
function formTrack(){const raw={};fields.forEach(f=>raw[f]=document.querySelector(`#${f}`)?.value?.trim?.()??"");checks.forEach(f=>raw[f]=document.querySelector(`#${f}`)?.checked===true);if(raw.releaseDate)raw.dateTbc=false;raw.bpm=Number(raw.bpm)||null;raw.price=Number(raw.price)||0;raw.sortPriority=Number(raw.sortPriority)||0;raw.previewStartSeconds=Math.max(0,Number(raw.previewStartSeconds)||0);raw.previewDurationSeconds=Number(raw.previewDurationSeconds);if(!Number.isFinite(raw.previewDurationSeconds)||raw.previewDurationSeconds<=0)raw.previewDurationSeconds=30;raw.moodTags=raw.moodTags.split(",").map(x=>x.trim()).filter(Boolean);raw.slug=raw.slug||slugify(raw.title);raw.productType="digital-track";return normaliseTrack(raw)}
function checklist(){const formValues=formTrack(),editingId=document.querySelector("#editingId")?.value,existing=findTrackByIdentity(editingId),assets=preservedAssetPayload(existing||{});if(document.querySelector("#cover")?.files[0])assets.coverUrl="pending";if(document.querySelector("#preview")?.files[0]){assets.previewUrl="pending";assets.previewPath="pending";assets.mp3Path="pending"}if(document.querySelector("#master")?.files[0]){assets.masterPath="pending";assets.wavPath="pending"}const t=normaliseTrack({...formValues,...assets}),h=trackHealth(t),readiness=trackReadiness(t),webMissing=Object.entries(readiness.website.checks).filter(([,complete])=>!complete).map(([field])=>missingFieldLabels[field]||field),useWeb=t.showInStore===true,score=useWeb?Math.round(readiness.website.complete/readiness.website.total*100):h.score,required=useWeb?webMissing:h.missingRequired.map(field=>missingFieldLabels[field]||field);document.querySelector("#releaseChecklist").innerHTML=`<strong>${score}% complete for ${escapeHtml(useWeb?"Website":t.status)}</strong><p>${required.length?`Required: ${escapeHtml(required.join(", "))}`:"Mandatory fields complete."}</p>${!useWeb&&h.missingRecommended.length?`<small>Recommended: ${escapeHtml(h.missingRecommended.join(", "))}</small>`:""}`;const summary=document.querySelector("#trackEditorReadinessSummary");if(summary){const card=(label,group)=>{const missing=Object.entries(group.checks).filter(([,complete])=>!complete).map(([field])=>missingFieldLabels[field]||field);const state=!group.enabled?"off":group.ready?"ready":"work";return`<article class="${state}" title="${escapeHtml(missing.join(", ")||"Complete")}"><strong>${label}</strong><span>${group.enabled?`${group.complete}/${group.total}`:"Off"}</span><small>${escapeHtml(missing.slice(0,2).join(", ")||"Complete")}${missing.length>2?` +${missing.length-2}`:""}</small></article>`};summary.innerHTML=card("Web",readiness.website)+card("Sale",readiness.sale)+card("DJ",readiness.dj)+card("Release",readiness.release)}return h}form.addEventListener("input",checklist);
function clearForm(){form.reset();document.querySelector("#editingId").value="";document.querySelector("#price").value="1.29";document.querySelector("#sortPriority").value="0";const dateTbc=document.querySelector("#dateTbc");if(dateTbc)dateTbc.checked=true;document.querySelector("#editorTitle").textContent="Add track";document.querySelector("#uploadProgress").textContent="";window.dispatchEvent(new Event("play-track-editor-clear-media"));checklist()}
function closeTrackEditor({confirmDiscard=true}={}){if(confirmDiscard&&editorDirty&&!confirm("Discard unsaved track changes?"))return false;form.reset();document.querySelector("#editingId").value="";document.querySelector("#editorTitle").textContent="Add track";document.querySelector("#uploadProgress").textContent="";document.querySelector("#trackEditor").hidden=true;window.dispatchEvent(new Event("play-track-editor-cancel"));editorSaveIntent=false;editorDirty=false;return true}
function openNewTrack(){window.dispatchEvent(new Event("play-track-editor-clear-media"));clearForm();editorSaveIntent=false;editorDirty=false;document.querySelector("#trackEditor").hidden=false;document.querySelector("#track-group-basics")?.setAttribute("open","");window.dispatchEvent(new CustomEvent("play-track-editor-mode",{detail:{mode:"new"}}));document.querySelector("#trackEditor").scrollIntoView({behavior:"smooth",block:"start"})}
function editTrack(id){const t=findTrackByIdentity(id);if(!t)return;const docId=trackDocumentId(t);window.dispatchEvent(new Event("play-track-editor-clear-media"));form.reset();editorSaveIntent=false;editorDirty=false;document.querySelector("#trackEditor").hidden=false;document.querySelector("#editingId").value=docId;fields.forEach(f=>{const input=document.querySelector(`#${f}`);if(input)input.value=f==="moodTags"?(t.moodTags||[]).join(", "):t[f]??""});checks.forEach(f=>{const input=document.querySelector(`#${f}`);if(input)input.checked=f==="dateTbc"&&t.releaseDate?false:Boolean(t[f])});document.querySelector("#editorTitle").textContent=`Edit ${t.title}`;document.querySelector("#uploadProgress").textContent="";window.dispatchEvent(new CustomEvent("play-track-editor-mode",{detail:{mode:"edit",id:docId,track:t}}));checklist();document.querySelector("#trackEditor").scrollIntoView({behavior:"smooth",block:"start"})}
globalThis.playOpenNewTrackEditor=openNewTrack;
globalThis.playOpenTrackEditor=editTrack;
document.addEventListener("click",event=>{const add=event.target.closest("#newTrack"),edit=event.target.closest("[data-edit],[data-library-edit]"),save=event.target.closest("#saveTrack,[data-save-track-top]"),close=event.target.closest("#closeEditor,[data-cancel-track-edit]");if(save){editorSaveIntent=true;setTimeout(()=>editorSaveIntent=false,0);return}if(close){event.preventDefault();event.stopImmediatePropagation();closeTrackEditor();return}if(!add&&!edit)return;event.preventDefault();event.stopImmediatePropagation();if(add)openNewTrack();else editTrack(edit.dataset.edit||edit.dataset.libraryEdit)},true);
form.addEventListener("input",()=>{if(!document.querySelector("#trackEditor").hidden)editorDirty=true});
form.addEventListener("keydown",event=>{if(event.key!=="Enter"||event.target.tagName==="TEXTAREA"||event.target.closest("#saveTrack,[data-save-track-top]"))return;event.preventDefault();event.stopPropagation()});
function upload(path,file,label,{returnDownloadUrl=true,inputId=""}={}){return new Promise((resolve,reject)=>{const task=uploadBytesResumable(ref(storage,path),file,{contentType:file.type});task.on("state_changed",s=>{const percent=s.totalBytes?Math.round(s.bytesTransferred/s.totalBytes*100):0;setSaveStage(`Uploading ${label}… ${percent}%`);if(inputId)setAssetProgress(inputId,label,percent,`Uploading ${label}…`)},error=>{if(inputId)setAssetProgress(inputId,label,0,`${label} upload failed. Existing assignment kept.`);reject(new Error(`${label} upload failed. Existing assignment kept. ${error.message||error}`))},async()=>{try{if(inputId)finishAssetProgress(inputId,label,`${label} upload complete`);resolve(returnDownloadUrl?await getDownloadURL(task.snapshot.ref):path)}catch(error){reject(new Error(`${label} uploaded, but its saved reference could not be resolved: ${error.message}`))}})})}
...
function mediaAction(inputId){return document.querySelector(`#${inputId}`)?.closest(".file-field")?.dataset.mediaAction||"keep"}
function clearArtworkAssets(assets){["coverUrl","coverPath","thumbnail"].forEach(field=>delete assets[field])}
function clearMp3Assets(assets){["previewUrl","previewPath","mp3Path","mp3Url","url"].forEach(field=>delete assets[field])}
function clearWavAssets(assets){["masterPath","wavPath"].forEach(field=>delete assets[field])}
...
form.addEventListener("submit",async e=>{
  e.preventDefault();
  if(!editorSaveIntent){
    progress.textContent="Use Save Draft or Save Track to create or update a track.";
    return;
  }
  editorSaveIntent=false;
  const buttons=[document.querySelector("#saveTrack"),document.querySelector("[data-save-track-top]")].filter(Boolean);
  buttons.forEach(button=>button.disabled=true);
  setTrackFormBusy(true);
  clearAssetProgress();
  try{
    setSaveStage("Preparing track…");
    const titleField=document.querySelector("#title");
    if(!titleField?.value.trim()){markRequiredFields(["title"],"Title is required before saving a track.");throw new Error("Title is required before saving a track.")}
    const track=formTrack(),editingId=document.querySelector("#editingId").value;
    const existing=findTrackByIdentity(editingId);
    if(editingId&&!existing)throw new Error("This edit session no longer matches a known track. Close the editor and reopen the track before saving.");
    if(!existing&&findTrackByIdentity(track.slug))throw new Error(`A track already exists with the slug/document ID "${track.slug}". Change the slug before saving a new track.`);
    let existingRecord=existing?{...existing}:null;
    if(existing){
      const snapshot=await getDoc(doc(db,"tracks",trackDocumentId(existing)));
      if(snapshot.exists())existingRecord={id:snapshot.id,...snapshot.data()};
    }
    const documentId=existing?trackDocumentId(existing):track.slug;
    if(!documentId)throw new Error("Track cannot be saved without a title or document ID.");
    ...
    const coverAction=cover?"replace":mediaAction("cover"),previewAction=preview?"replace":mediaAction("preview"),masterAction=master?"replace":mediaAction("master");
    const assets=preservedAssetPayload(existingRecord||{});
    if(coverAction==="remove")clearArtworkAssets(assets);
    if(previewAction==="remove")clearMp3Assets(assets);
    if(masterAction==="remove")clearWavAssets(assets);
    const manualWavPath=document.querySelector("#wavPath")?.value.trim()||"";
    const manualMp3Url=document.querySelector("#mp3Url")?.value.trim()||"";
    if(masterAction!=="remove"&&manualWavPath&&manualWavPath!==String(existingRecord?.wavPath||"")){assets.wavPath=manualWavPath;assets.masterPath=manualWavPath}
    if(previewAction!=="remove"&&manualMp3Url&&manualMp3Url!==String(existingRecord?.mp3Url||""))assets.mp3Url=manualMp3Url;
    ...
    if(["coming-soon","published"].includes(track.status)&&track.showInStore&&health.missingRequired.length){const label=track.status==="coming-soon"?"Coming Soon":"Published",message=`Cannot save ${label} with Website On. Missing required fields: ${health.missingRequired.join(", ")}`;markRequiredFields(health.missingRequired,message);throw new Error(message)}
    const stamp=Date.now();
    if(cover){assets.coverPath=`covers/${track.slug}-${stamp}.${cover.name.split(".").pop()}`;assets.coverUrl=await upload(assets.coverPath,cover,"Artwork",{inputId:"cover"});assets.thumbnail=assets.coverUrl}
    if(master){assets.masterPath=`masters/${track.slug}-${stamp}.wav`;assets.wavPath=assets.masterPath;if(!preview){setSaveStage("Making MP3 preview from WAV…");preview=await previewFrom(master)}await upload(assets.masterPath,master,"WAV",{returnDownloadUrl:false,inputId:"master"})}
    if(preview){assets.previewPath=`previews/${track.slug}-${stamp}.mp3`;assets.previewUrl=await upload(assets.previewPath,preview,"MP3",{inputId:"preview"});assets.mp3Path=assets.previewPath;assets.mp3Url=assets.previewUrl;assets.url=assets.previewUrl}
    const payload={...track};
    [...protectedAssetFields,"id","firestoreId","docId","documentId","legacyId","createdAt","updatedAt"].forEach(field=>delete payload[field]);
    ...
    setSaveStage("Saving track details…");
    await setDoc(doc(db,"tracks",documentId),payload,{merge:true});
    editorDirty=false;
    setSaveStage(`Track saved successfully. Saved as ${documentId}.`);
    window.dispatchEvent(new CustomEvent("play-track-editor-saved",{detail:{id:documentId}}));
