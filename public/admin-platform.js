...
function openNewTrack(){clearForm();editorSaveIntent=false;editorDirty=false;document.querySelector("#trackEditor").hidden=false;document.querySelector("#track-group-basics")?.setAttribute("open","");window.dispatchEvent(new CustomEvent("play-track-editor-mode",{detail:{mode:"new"}}));document.querySelector("#trackEditor").scrollIntoView({behavior:"smooth",block:"start"})}
function editTrack(id){const t=findTrackByIdentity(id);if(!t)return;const docId=trackDocumentId(t);form.reset();editorSaveIntent=false;editorDirty=false;document.querySelector("#trackEditor").hidden=false;document.querySelector("#editingId").value=docId;fields.forEach(f=>{const input=document.querySelector(`#${f}`);if(input)input.value=f==="moodTags"?(t.moodTags||[]).join(", "):t[f]??""});checks.forEach(f=>{const input=document.querySelector(`#${f}`);if(input)input.checked=f==="dateTbc"&&t.releaseDate?false:Boolean(t[f])});document.querySelector("#editorTitle").textContent=`Edit ${t.title}`;document.querySelector("#uploadProgress").textContent="";window.dispatchEvent(new CustomEvent("play-track-editor-mode",{detail:{mode:"edit",id:docId,track:t}}));checklist();document.querySelector("#trackEditor").scrollIntoView({behavior:"smooth",block:"start"})}
globalThis.playOpenNewTrackEditor=openNewTrack;
globalThis.playOpenTrackEditor=editTrack;
document.addEventListener("click",event=>{const add=event.target.closest("#newTrack"),edit=event.target.closest("[data-edit],[data-library-edit]"),save=event.target.closest("#saveTrack,[data-save-track-top]"),close=event.target.closest("#closeEditor");if(save){editorSaveIntent=true;setTimeout(()=>editorSaveIntent=false,0);return}if(close){event.preventDefault();event.stopImmediatePropagation();const isNew=!document.querySelector("#editingId").value;if(editorDirty&&isNew&&!confirm("Discard unsaved new track?"))return;form.reset();document.querySelector("#editingId").value="";document.querySelector("#editorTitle").textContent="Add track";document.querySelector("#uploadProgress").textContent="";document.querySelector("#trackEditor").hidden=true;editorSaveIntent=false;editorDirty=false;return}if(!add&&!edit)return;event.preventDefault();event.stopImmediatePropagation();if(add)openNewTrack();else editTrack(edit.dataset.edit||edit.dataset.libraryEdit)},true);
