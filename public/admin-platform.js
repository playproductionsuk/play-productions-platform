...
function openNewTrack(){clearForm();document.querySelector("#trackEditor").hidden=false;document.querySelector("#track-group-web")?.setAttribute("open","");window.dispatchEvent(new CustomEvent("play-track-editor-mode",{detail:{mode:"new"}}));document.querySelector("#trackEditor").scrollIntoView({behavior:"smooth",block:"start"})}
function editTrack(id){const t=state.tracks.find(x=>String(x.id)===String(id)||String(x.slug)===String(id)||String(x.legacyId)===String(id));if(!t)return;form.reset();document.querySelector("#trackEditor").hidden=false;document.querySelector("#editingId").value=t.id;fields.forEach(f=>{const input=document.querySelector(`#${f}`);if(input)input.value=f==="moodTags"?(t.moodTags||[]).join(", "):t[f]??""});checks.forEach(f=>{const input=document.querySelector(`#${f}`);if(input)input.checked=f==="dateTbc"&&t.releaseDate?false:Boolean(t[f])});document.querySelector("#editorTitle").textContent=`Edit ${t.title}`;document.querySelector("#uploadProgress").textContent="";window.dispatchEvent(new CustomEvent("play-track-editor-mode",{detail:{mode:"edit",id:t.id}}));checklist();document.querySelector("#trackEditor").scrollIntoView({behavior:"smooth",block:"start"})}
globalThis.playOpenNewTrackEditor=openNewTrack;
globalThis.playOpenTrackEditor=editTrack;
document.addEventListener("click",event=>{const add=event.target.closest("#newTrack"),edit=event.target.closest("[data-edit],[data-library-edit]");if(!add&&!edit)return;event.preventDefault();event.stopImmediatePropagation();if(add)openNewTrack();else editTrack(edit.dataset.edit||edit.dataset.libraryEdit)},true);
document.querySelector("#closeEditor").addEventListener("click",()=>document.querySelector("#trackEditor").hidden=true);
